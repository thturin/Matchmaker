...
count the moves
display a message and close the message window
add more symbols and make the grid larger 
"""
import random, time
...
def close_window(self):
    root.destroy() #command closes the window
...
def show_symbol(x, y):
    ...
    print(buttons[x, y][
              'text'])  # the button object contains the characterstic (text=) so it can be called this way Button(text='unicode',etc)
    ...
    if first:  # if its the first turn
        ...
        if buttons[previousX, previousY]['text'] != buttons[x, y]['text']:  # if the symbols do not match
            ...
            buttons[previousX, previousY][
                'command'] = DISABLED  # disable matching buttons so player can't press them again
...
for x in range(6):  # range is from 0->5
    for y in range(4):
        button = Button(command=lambda x=x, y=y: show_symbol(x, y), width=3,
                        height=3)  # BUTTON(command-function or method to be called when the button is clicked)
